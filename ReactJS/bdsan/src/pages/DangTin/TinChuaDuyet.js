...
function TinBDS() {
    ...
    const loadTinBDS = async () => {
        const result = await axios.get('http://localhost:3001/batdongsan/user/notapproved',{ headers: authHeader() });
                        ...
                        { (tinbds !="") ?
                            tinbds.map((tinbds, index) => (
                                <tr key={index}>
                                    ...
                                    <td className='pt-2' ><span className={styles.btnNot}>{tinbds.BDS_TrangThai}</span></td>
