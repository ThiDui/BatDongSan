...
function EditTinBds(){
    ...
    // console.log("load",formValues.BDS_MoTaChiTiet)

    // const loadBds = async () => {
    //     const result = await axios.get(`http://localhost:3001/batdongsan/${id}`);
    ...
    //         .then((result) => {
    //             const dt = result.data.filter((a) => a.TinhThanhTTMa == id)
    //             setQuanhuyen(dt);
    //         })
    ...
    //         .then((result) => {
    //             const dt = result.data.filter((a) => a.QuanHuyenQHMa == id)
    //             setXaphuong(dt);
    //         })
                                ...
                                <Form.Select aria-label="Default select example" onChange={(e) => handleTinhthanh(e.target.value)} >
                                    ...
                                    {quanhuyen && quanhuyen !== undefined ?
                                        tinhthanh.map((tt, index) => {
                                            return (
                                                <option key={index} value={tt.TT_Ma}>{tt.TT_Ten}</option>
                                            )
                                ...
                                <Form.Select aria-label="Default select example" onChange={(e) => handleQuanhuyen(e.target.value)} >
                                    ...
                                    {quanhuyen && quanhuyen !== undefined ?
                                        quanhuyen.map((qh, index) => {
                                            return (
                                                <option key={index} value={qh.QH_Ma}>{qh.QH_Ten}</option>
                                            )
                                ...
                                <Form.Select aria-label="Default select example" name="XaPhuongXPMa" onChange={handleChange}>
                                    ...
                                    {xaphuong && xaphuong !== undefined ?
                                        xaphuong.map((tt, index) => {
                                            return (
                                                <option key={index} value={tt.XP_Ma}>{tt.XP_Ten}</option>
                                            )
